...
Usage:
    ...
    python3 queue_runner.py --concurrent 3 --batch 50  # 3 concurrent, batch 50

Optimized:
    - Parallel task processing (configurable concurrency)
    - Shared HTTP connection pool across tasks
    - Skip novels that already have chapters (fast-track)
    - Reduced delays (0.1s between task starts)
    - Batch DB commits
"""
import asyncio
import logging
...
async def process_one(task_id: str, novel_title: str, remaining: int) -> bool:
    """Process a single crawl task in its own DB session. Returns True on success."""
...
async def main():
    ap = ArgumentParser()
    ap.add_argument("--concurrent", type=int, default=DEFAULT_CONCURRENCY, help="Concurrent tasks")
    ...
    log.info(f"Queue runner started (concurrent={args.concurrent}, batch={batch_limit or 'all'})")
    ...
    while True:
        ...
        async with async_session_factory() as db:
            ...
            # Resolve novel titles for logging
        ...
        log.info(
            f"  📊 {completed} ok + {failed} fail = {total_processed} total "
